...
const EditQuoteModal = ({ showEditModal, setShowEditModal, selectedQuote, onEditSuccess, setOpenSnackbar }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:8087/sales/AllQuotes/${selectedQuote.id}`, formData);
...
const DeleteQuoteModal = ({ showDeleteModal, setShowDeleteModal, handleDelete, selectedQuote, setOpenSnackbar }) => {
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await handleDelete(selectedQuote); // Pass the selected quote object instead of just the ID
      setOpenSnackbar(true);
    } catch (error) {
...
const AllQuotesPage = () => {
  ...
  const handleDelete = async (quote) => {
    console.log('Selected Quote for deletion:', quote);
    try {
      const response = await axios.delete(`http://localhost:8087/sales/DeleteQuote/${quote.id}`);
      if (response.status === 200) {
        setShowDeleteModal(false);
        // Fetch updated quote data after deletion
        try {
          const updatedResponse = await axios.get('http://localhost:8087/sales/AllQuotes');
  ...
  const handleEditSuccess = (updatedQuote) => {
    // Update the quotes state with the updated quote
                    ...
                    <tbody>
                      {quotes.map((quote) => (
                        <tr key={quote.id}>
                          ...
                          <td>{quote.notes}</td>
                          <td>
                            <button type="button" className="edit-button" onClick={() => handleEdit(quote.id)}>
